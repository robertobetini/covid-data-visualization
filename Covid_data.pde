...
String textBetween(String s, String start, String end) {
...
void draw() {
  background(255);
  int maxP = height - 200;
  float w = 0.025 * width, off = width / 10;
  ...
  for(int i = 0; i < data.length; i++) {
    if(data[i].name == "FAILED") {
      text("Failed loading", 2.25*off + 2*i*w + i*off, 20);
    } else {
      text(data[i].name, 2.25*off + 2*i*w + i*off, 20);
      text(data[i].cases + " cases", 2.25*off + 2*i*w + i*off, 40);
    }
  }
  pop();
  ...
  textSize(14);
  fill(195);
  rect(1.1*off, 10, 150, 80);
  fill(180, 0, 0);
  rect(1.2*off, 20, 30, 20);
  fill(0);
  text("Death rate", 1.2*off + 35, 35);
  fill(0, 180, 0);
  rect(1.2*off, 60, 30, 20);
  fill(0);
  text("Recovery rate", 1.2*off + 35, 75);
  pop();
  
  //drawing the avg and std dev
  push();
  textAlign(LEFT);
  text("AVG world death rate = " + float(round(avg_dRate * 1000)) / 1000, 2.3*off, 50);
  text("STD DEV world death rate = " + float(round(std_dev[0] * 1000)) / 1000, 2.3*off + 210, 50);
  text("AVG world recovery rate = " + float(round(avg_rRate * 1000)) / 1000, 2.3*off, 70);
  text("STD DEV world recovery rate = " + float(round(std_dev[1] * 1000)) / 1000, 2.3*off + 210, 70);
  pop();
  push();
  textSize(24);
  textAlign(RIGHT);
